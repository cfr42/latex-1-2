  ...
  \resizebox{\linewidth}{!}{%
    \begin{tikzpicture}[thick, >=Latex, font=\sffamily]
      \begin{scope}[minimum width=1.5cm,minimum height=1.5cm,inner sep=.1em]
        ...
        \path (3.2,1) node[penderfyniad] (errors) {\shortstack{Any\\Errors?}};
        \path (6.8,1) node[penderfyniad] (bibtexerrors) {\shortstack{Any\\Errors?}};
        \path (10,1) node[penderfyniad] (errors2) {\shortstack{Any\\Errors?}};
      ...
      \draw[->] (edit) -- (1,1) -- (errors) node[midway,above] {\shortstack{pdf\LaTeX{}\\\texttt{.tex} file}};
      \draw (editbib) -- (1,1);
      \draw[->] (errors) -- ++(0,-2.25) node[midway,left] {Yes} -| (edit) node[pos=0,below,anchor=north east] {Error in document};
      \draw[->] (errors) -- (bibtexerrors) node[midway,above] {\shortstack{No errors.\\Run Biber}};
      \draw[->] (bibtexerrors) -- ++(0,-3) node[midway,right] {Yes} -| (edit) node[pos=0,below,anchor=north east] {Misspelt/missing label or bib style};
      \draw[->] (bibtexerrors) -- ++(0,2.5) node[midway,right] {Yes} -| (editbib) node[pos=0,above,anchor=south east] {Error in bib file};
      \draw[->] (bibtexerrors) -- (errors2) node[midway,above] {\shortstack{No errors.\\Run pdf\LaTeX{}\\(twice)}};
...
\begin{exercise}\label{ex:biblatex-example}\tutornote{Ref.: \cref{sec:samples}.}
  ...
  There are various options you can pass to the \pkg{biblatex} package that affect the formatting.
